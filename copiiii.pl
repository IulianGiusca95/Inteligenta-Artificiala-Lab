...
ales(X):-copil(X,_,I,K),once((I>=1.60, I =< 1.70; K>=50,K =< 80)).
...
lista_b(L):-bagof(X,I^K^copil(X,b,I,K),L).
ales_b(X):-copil(X,b,_,_).
lista_b2(L):-setof(X,ales_b(X),L).
lista_baschet(L):-findall(X,(copil(X,_,I,_),I>1.60),L).
lista_i(L):-setof(X,G1^G2^G3^I^(copil(G1,G2,I,G3),tip_inaltime(I,X)),L).
lista_ii(C,L):-setof(X,G2^G3^I^(copil(X,G2,I,G3),tip_inaltime(I,C)),L).
