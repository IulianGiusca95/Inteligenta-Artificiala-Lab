...
%stare(+Mal,+NrMisionariMalCurent,+NrCanibaliMalCurent,+NrMisionariMalOpus,+NrCanibaliMalOpus,+CapacitateaBarcii)
initial(stare(est, 3,3 ,0 ,0 ,2)).%starea initiala
scop(stare(vest, 3, 3, 0, 0, 2)).%starea finala
...
s(stare(B, NMB, NCB, NMis, NCan, M),
  stare(B1, NMB1, NCB1, NMis1, NCan1, M)) :- M1 is min(M, NMB), M2 is min(M, NCB),%M1:nr maxim de misionari in barca, M2:canibali
...
afis(stare(B, NMB, NCB, NMis, NCan, M)) :- write('Barca se afla pe malul de '), write(B), nl,
                        ...
                        write('  pe malul opus se afla '), write(NMis), write(' misionari si '), write(NCan), write(' canibali.'), nl,nl.
